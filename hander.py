...
class ClassIndex(object):
    def GET(self, classid, pageid):
        article_list = web.ctx.orm.query(Article).filter_by(classid=int(classid)).order_by(
            Article.posttime.desc()).all()
        content_classes = web.ctx.orm.query(Classes).all()
        article_list, page_count = Pager(article_list, int(pageid))
        return global_render.index(article_list=article_list, classes=content_classes, page_count=page_count,
                                   page=int(pageid))
...
class Login(object):
    ...
    def POST(self):
        ...
        if hashlib.md5(password).hexdigest() == user.userpass:
            web.config._session = True
